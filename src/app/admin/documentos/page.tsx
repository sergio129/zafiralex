                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {new Date(doc.createdAt).toLocaleDateString()}
                        ...
                        onClick={() => {
                          setViewDocument(doc);
                          setShowPreview(true);
                          setHoverPreview(null); // Cerrar vista previa al abrir la vista completa
                        }}
                        onMouseEnter={(e) => {
                          // Mostrar la vista previa con un pequeño retraso para evitar parpadeos
                          ...
                          // Almacenar el temporizador en un atributo del elemento
                          (e.target as HTMLElement).setAttribute('data-timer', String(timer));
                        }}
                        onMouseLeave={(e) => {
                          // Cancelar el temporizador si existe
                          const timerId = (e.target as HTMLElement).getAttribute('data-timer');
                          if (timerId) {
                            clearTimeout(Number(timerId));
                            (e.target as HTMLElement).removeAttribute('data-timer');
                          }
                          
                          // No cerramos inmediatamente para permitir moverse al popup
                          // La vista previa se cierra con el botón de cerrar
                        }}
              ...
              <div className="w-full md:w-2/3 flex-1 overflow-hidden flex flex-col">
                {viewDocument.mimeType?.includes('image/') ? (
                  <div className="flex-1 overflow-auto flex items-center justify-center bg-gray-100 p-4">
                    ...
                    />
                  </div>                ) : viewDocument.mimeType === 'application/pdf' ? (
                  <iframe 
                    src={`${viewDocument.fileUrl}#toolbar=0&navpanes=0&scrollbar=1`}
      ...
      {hoverPreview && (
        ...
        >
          <div className="p-3 border-b bg-gray-50 flex justify-between items-center">
            <div className="text-base font-medium text-gray-800">{hoverPreview.doc.title}</div>
            <button 
          ...
          <div className="p-4 flex-1">
            {hoverPreview.doc.mimeType?.includes('image/') ? (
              ...
              <div className="h-[400px] w-full bg-gray-50 rounded-md overflow-hidden">
                <iframe 
                  src={`${hoverPreview.doc.fileUrl}#toolbar=0&navpanes=0&scrollbar=0`}
                  className="w-full h-full border-0" 
              ...
              <div className="flex flex-col items-center justify-center h-[400px] bg-gray-50 rounded-md p-4">
                <svg className="h-16 w-16 text-blue-600" fill="currentColor" viewBox="0 0 384 512">
                ...
                <h3 className="mt-4 text-lg font-medium text-gray-800">Documento Word</h3>
                <p className="mt-2 text-gray-600">
                  No se puede mostrar una vista previa de este documento Word
                </p>
                <a 
              ...
              <div className="flex flex-col items-center justify-center h-[400px] bg-gray-50 rounded-md p-4">
                <svg className="h-16 w-16 text-green-600" fill="currentColor" viewBox="0 0 384 512">
                ...
                <h3 className="mt-4 text-lg font-medium text-gray-800">Hoja de cálculo Excel</h3>
                <p className="mt-2 text-gray-600">
                  No se puede mostrar una vista previa de esta hoja de cálculo
                </p>
                <a 
                  href={hoverPreview.doc.fileUrl}
                  target="_blank"
              ...
              <div className="flex flex-col items-center justify-center h-[400px] bg-gray-50 rounded-md p-4">
                <svg className="h-16 w-16 text-gray-500" fill="currentColor" viewBox="0 0 384 512">
                  <path d="M369.9 97.9L286 14C277 5 264.8-.1 252.1-.1H48C21.5 0 0 21.5 0 48v416c0 26.5 21.5 48 48 48h288c26.5 0 48-21.5 48-48V131.9c0-12.7-5.1-25-14.1-34zM332.1 128H256V51.9l76.1 76.1zM48 464V48h160v104c0 13.3 10.7 24 24 24h104v288H48z"/>
                </svg>
                <h3 className="mt-4 text-lg font-medium text-gray-800">
                  {hoverPreview.doc.fileName.split('.').pop()?.toUpperCase() || 'Documento'}
                </h3>
                <p className="mt-2 text-gray-600">
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="mt-4 px-4 py-2 bg-gray-600 text-white text-sm rounded-md hover:bg-gray-700"
                >
                  Abrir archivo
                </a>
              </div>
          ...
          <div className="p-3 border-t bg-gray-50 flex justify-between items-center text-sm text-gray-600">
            <div>
              <span className="font-medium">Ref:</span> {hoverPreview.doc.documentRef}
            </div>
            <div>
